...
case class Hive[A](action: (HiveConf, IMetaStoreClient) => Result[A]) {
  ...
  def flatMap[B](f: A => Hive[B]): Hive[B] =
    Hive((conf, client) => action(conf, client) match {
      case Error(e) => Error(e)
      case Ok(a)    => f(a).action(conf, client)
    })

  /** Chain an unsafe operation through a successful Hive operation. */
  def safeMap[B](f: A => B): Hive[B] =
    flatMap(a => Hive.value(f(a)))
...
object Hive {
  ...
  def existsTableStrict[T <: ThriftStruct : Manifest](
    ...
    val actualCols     = sd.getCols.asScala.map(c => (c.getName.toLowerCase, c.getType.toLowerCase))
    val actualPartCols = actualTable.getPartitionKeys.asScala.map(c => (c.getName.toLowerCase, c.getType.toLowerCase))
